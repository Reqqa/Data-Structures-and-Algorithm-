...
public class Main {
    ...
    public static List<Schedule> generateRandom(int count,
        ...
        List<String> names = new ArrayList<>(Arrays.asList(proj_name));
        Collections.shuffle(names);

        for (int i = 0; i < count; i++) {
            String id       = "J" + (i + 1);
            String name     = (i < names.size()) ? names.get(i) : "Project " + (i + 1);
            String sector   = first_name[rnd.nextInt(first_name.length)];
    ...
    static void printOutput(String algoName, List<Schedule> all, List<Schedule> selected, double totalProfit, int totalJobsCreated) {
        Set<String> selectedSet = new HashSet<>();
        for (Schedule s : selected) selectedSet.add(s.getId());
        List<Schedule> unselected = new ArrayList<>();
        for (Schedule s : all)
            if (!selectedSet.contains(s.getId())) unselected.add(s);
        System.out.println("\n");
